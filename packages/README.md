...
### Navigation within Microfrontends

### Few Rquirements

1. Routing between microfrontends should be independent and not interfere with each other.
2. Each microfrontend should handle its own routing and navigation.

#### Navigation within Microfrontends

React Router is used for routing within each microfrontend. React Router has 3 types :

1. BrowerRouter: Uses the HTML5 history API to manage navigation and URLs. This updates the URL in the browser without reloading the page.
2. HashRouter: Uses the URL hash to manage navigation. This is useful for static file
3. MemoryRouter: Keeps the history of the navigation in memory, useful for testing or non-browser environments.The URL is not updated and navigation is handled internally within the application.

In our Project we will use BrowserRouter for the container and memory router for the microfrontends.

#### Synchronizin Router history between microfrontends

The history object between the container and microfrontends need to be synchronized since container uses browser history and microfrontends use memory history. There are 2 options the history needs to be synchorinized:

1. User clicks from contianer application , in which case the history needs to be communicated down to the Marketing microfrontend ( Marketing Memory History should be updated with the current path ).
2. User clicks from the Marketing microfrontend, in which case the history needs to be communicated up to the container application ( Container's Browser History should be updated) .

##### 1. synch container history with microfrontend route history

1. In order for the container application to communicate with the Marketing microfrontend , the mount function now needs to return an object.. This object now contians a handler function called `onParentNavigate`. This function is used to update the microfrontend's router history whenever the parent application navigates to a new route.
2. In Marketing Component , we pass the `onParentNavigate` function to the `history.listen` method. This allows the microfrontend to be notified of any navigation events that occur in the container application.

##### 2. synch microfrontent route history with container history

1. In the `MarketingApp.js` file within the container app, we pass an `onNavigate` callback to the `mount` function. This callback is invoked whenever the pathname changes in the Marketing microfrontend.
2. Inside the `onNavigate` callback, we check if the new pathname is different from the current pathname in the container's history.
3. If they are different, we update the container's history using `history.push(newPathname)`, which updates the browser's URL and ensures that the container's history is in sync with the Marketing microfrontend's navigation.

In the Marketing Frontend , we register a eventlistener that triggers the `onNavigate` callback whenever the memory history changes. This allows the container app to be notified of any navigation events that occur within the Marketing microfrontend.

### Use of publicPath to resolve issues with nested paths

When working with microfrontends, especially when they are hosted on different paths, you may encounter issues with routing and asset loading. The `publicPath` configuration in Webpack helps resolve these issues by defining the base path for all assets within the application.

If publicpath is not defined , when the router routes to a path , it appens main.js to the path - https://hostname/microfrontend-name/main.js. This can lead to issues because main.js generated by webpack for that project is not available at that path. Its directly under http://jpstname/main.js. Inorder to resolve this issue we will need to specify the publicPath to "http://localhost:<oirt>/" so the application can correctly load the assets and handle routing without conflicts.

### The Route Component Ordering

When using React Router, the order of route components matters. The router will match the first route that matches the current URL, so more specific routes should be defined before more general ones.

### Lazy Loading of the components

To optimize the loading of microfrontends, we can use React's `lazy` and `Suspense` features to lazy load components. This allows us to load components only when they are needed, reducing the initial bundle size and improving performance.
